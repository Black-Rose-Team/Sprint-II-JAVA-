 ...
 * @author User
 */
public class DeleteProvider extends Form{
    Form current ;
              private Resources themeee;

    public DeleteProvider(Form p){
   ...
   getToolbar().addMaterialCommandToSideMenu("Exit", FontImage.MATERIAL_EXIT_TO_APP, e -> {Display.getInstance().exitApplication();}); 
    ...
    dlg.add(blueLabel);
    TextArea ta = new TextArea("Vous voulez vraiment Supprimer cette Calendrier");
...
btnsupp.addActionListener(e -> {
    ...
    dlg.showDialog();
       ToastBar.Status status = ToastBar.getInstance().createStatus();
            status.setMessage("Calendrier Supprimer ...");
    ...
    //////////////////////////////////////////////////////
        ok.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent evt) {
                ...
                {
                    try {
                        Provider t = new Provider(tfCin.getText());
                        if( ServiceProvider.getInstance().supprimerTask(t))
                            Dialog.show("Success","Calendrier Supprimer avec succés !",new Command("OK"));
                        else
