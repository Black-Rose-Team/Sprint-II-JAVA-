 ...
 * @author User
 */
public class DeleteCharge extends Form{
    Form current ;

    public DeleteCharge(Form p){

        setTitle("Supprimer un Calandrier");
   ...
   getToolbar().addMaterialCommandToSideMenu("Exit", FontImage.MATERIAL_EXIT_TO_APP, e -> {Display.getInstance().exitApplication();}); 
    ...
    dlg.add(blueLabel);
    TextArea ta = new TextArea("Are you sure to delete this Charge");
...
btnsupp.addActionListener(e -> {
            ...
            dlg.showDialog();
            status.setMessage("Delete the Charge ...");
            status.show();
    ...
    ok.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent evt) {
                ...
                {
                    try {
                        Charge t = new Charge(Integer.parseInt(tfCin.getText()));
                        if( ServiceCharge.getInstance().supprimerTask(t))
                            Dialog.show("Success","Charge has Been Deleted !",new Command("OK"));
                        else
                        {
                        ...
                        Dialog.show("ERROR","Charge Dosen't Exist", new Command("OK"));
                    }
                }
                     }});
                ...
                , e-> new ChargeGui(p).show()); // Revenir vers l'interface précédente
     }   
